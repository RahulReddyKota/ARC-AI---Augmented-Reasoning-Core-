...
- [NLP Analysis Pipeline](#nlp-analysis-pipeline)
...
The system includes a custom-designed web UI with multi-model switching (Llama 3.1 / Mistral / Qwen 2.5), streaming responses, conversation history, and an integrated NLP analysis panel that runs **9 techniques** on every answer at the click of a button.
...
- **9 NLP techniques** — intent classification, NER, topic modeling, extractive QA, sentiment analysis, text summarization, keyword extraction, readability scoring, and emotion detection.
...
1. User questions are embedded with **MiniLM**.
2. Embeddings are matched against chunked legal documents in **ChromaDB** via cosine similarity.
3. The top-K results are injected into a grounded prompt sent to a local LLM (**Ollama**).
4. The response streams back with inline citations (`[S1]`, `[S2]`, …).

An optional NLP analysis layer runs **9 techniques** (intent classification, NER, topic modeling, extractive QA, sentiment, summarization, keywords, readability, emotion) on the retrieved context.
...
Scrapes 7 Maryland sources, chunks into ~500-token pieces, embeds with MiniLM, and stores in ChromaDB. Takes **3–6 minutes** (one-time).
...
- Click **"Analyze (NLP)"** under any answer to expand the 9-technique analysis panel.
...
| POST   | `/api/nlp/analyze`        | NLP analysis (9 techniques)       |
...
- **Freshness** — Point-in-time scrape snapshot. Maryland law changes annually; scheduled re-scraping is needed for production.
